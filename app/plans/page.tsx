...
import { Plus, Layers, Dumbbell, Trash2, Calendar, X, Check, Edit3 } from 'lucide-react';
          ...
          {plans.length === 0 ? (
            ...
            plans.map((plan) => (
              ...
              >
                ...
                <div className="grid grid-cols-7 gap-1 pt-1 border-t border-gray-800">
                  {DAYS_OF_WEEK.map((dayName, idx) => {
                    const matchedDay = plan.workout_plan_days?.find((d) => d.day_of_week === idx);
                    const routineName = matchedDay?.workouts?.name;
                    return (
                      <div key={idx} className="bg-gray-950 border border-gray-800/60 p-1.5 rounded-lg text-center flex flex-col justify-between">
                        <span className="text-[10px] text-gray-500 font-medium">{dayName.slice(0, 3)}</span>
                        <span className={`text-[11px] font-medium truncate mt-1 ${routineName ? 'text-gray-200' : 'text-gray-600'}`}>
                          {routineName || 'Rest'}
                        </span>
                      </div>
                    );
